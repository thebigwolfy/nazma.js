...
class GiveawaysManager extends EventEmitter {
    ...
    start(channel, options) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(!channel || !channel.id) return reject(`NazmaJs - Giveaway - channel is not a valid guildchannel. (val=${channel})`);
			
            if(!options.time || isNaN(options.time)) return reject(`options.time is not a number. (val=${options.time})`);
			
            if(!options.prize) return reject(`options.prize is not a string. (val=${options.prize})`);
			
            if(!options.winnerCount || isNaN(options.winnerCount)) return reject(`options.winnerCount is not a number. (val=${options.winnerCount})`);
        ...
        if(!storageExists) {
            ...
            try{
                ...
                if(Array.isArray(giveaways)) {
                    ...
                    throw new SyntaxError('NazmaJS - Giveaway - The storage file is not properly formatted (giveaways is not an array).');
                ...
                if(e.message === 'Unexpected end of JSON input') {
                    ...
                    throw new SyntaxError('NazmaJS - Giveaway - The storage file is not properly formatted (Unexpected end of JSON input).');
    ...
    _checkGiveaway() {
        ...
        this.giveaways.forEach(async (giveaway) => {
            ...
            if(giveaway.remainingTime < this.options.updateCountdownEvery) setTimeout(() => this.end.call(this, giveaway.messageID), giveaway.remainingTime);
